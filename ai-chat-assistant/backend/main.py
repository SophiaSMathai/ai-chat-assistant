...
Exposes:
  POST /api/chat   -> send a user message, get the AI reply
  POST /api/clear  -> reset a session's conversation history
  GET  /api/health -> simple liveness check (handy for Render/Railway)
...
# Catches anything unexpected so the frontend always gets clean JSON
